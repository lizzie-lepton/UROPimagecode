...
def match_scales(nuv, z, antennas, galaxy):
    galaxy = np.real(galaxy)
    antennas = antennas()
    nmin, nmax, N = scale_uv(antennas, z, nuv)
    resampledang = scalebox(z,nuv)
    zoomfactor = nuv/200
    scaledsky = scipy.ndimage.interpolation.zoom(galaxy, 1.5)
    return scaledsky
...
def max_baseline(antennas):

	baselines = []
	for (x, y) in antennas:
		baseline_length = np.sqrt(x**2 + y**2)
		baselines.append(baseline_length)
	bmax = max(baselines)
	return bmax

def min_baseline(antennas):
...
def small_interferometer(nuv, z): 
    ...
    scaledsky = match_scales(nuv, z, telescope, ftgal)

    sampled_sky = sample(scaledsky,gridded)
    ...
    difference = difference_plot(scaledsky, deconvolved)
...
# so FT of width of antenna array is the size in Fspace of the beam
    ...
    angular_width = (lengthx / 1e6) * (180/ np.pi) * (60)
    
    
    print "The size of the beam is ", angular_width, " arcminutes"
